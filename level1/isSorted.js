...
//use a sort function on the array, introduce some if conditionals to make sure it can register duplicates correctly
...
function isSorted(arr) {
    ...
    for (let i = 0; i < order.length; i++) {
        if (arr[i] !== order[i]) {
            console.log(`The proper ascending order is ${order}`)
            return false;
        }
    }
    return true
...
console.log(isSorted([26, 6, 72, -12]));
